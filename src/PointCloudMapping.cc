...
namespace ORB_SLAM3
{
    ...
    void PointCloudMapping::shutdown()
    {
        ...
        // viewerThread->join();
        save();
        std::cout << "Point cloud saved to ./PointCloudmapping.pcd" << std::endl;
    }
    ...
    void PointCloudMapping::insertKeyFrame(KeyFrame *kf, cv::Mat &color, cv::Mat &depth)
    {
        ...
        lck.unlock(); // 手动解锁（已完成数据写入，无需持有锁）
        // 替代原viewer线程的点云生成逻辑，避免依赖线程
        generateAndFusePointCloud();
        // 线程阻塞
        keyFrameUpdated.notify_one();
    }
    ...
    pcl::PointCloud<PointCloudMapping::PointT>::Ptr PointCloudMapping::GetPointCloud(KeyFrame *kf, cv::Mat &left, cv::Mat &right, cv::Mat &disp, cv::Mat &Q)
    {
        ...
        PointCloud::Ptr cloud(new PointCloud());
        pcl::transformPointCloud(*tmp, *cloud, kf->GetPose().matrix());
        cloud->is_dense = false;
...
// --------------- 新增：点云生成+融合核心函数（替代原viewer线程）---------------
    void PointCloudMapping::generateAndFusePointCloud()
    {
        size_t N = keyframes.size();
        if (N <= lastKeyframeSize) return;
        ...
        std::cout << "Global point cloud size after filter: " << globalMap->points.size() << std::endl;
        lastKeyframeSize = N;
    }
    ...
    void PointCloudMapping::save()
    {
        if (this->globalMap != nullptr)
        {
            ...
            std::cout << "Dense point cloud saved successfully! Path: ./PointCloudmapping.pcd" << std::endl;
            std::cout << "Total points: " << globalMap->points.size() << std::endl;
        }
        else
        {
            std::cerr << "Warning: Global point cloud is empty, save failed!" << std::endl;
        }
    }
}